...
def display_listing(request, listing_id):
    ...
    print(is_listing_in_watchlist)
    is_owner = request.user.username == listing.owner.username
    print(is_owner)
    similar_list = Listing.objects.filter(category=listing.category).exclude(pk=listing_id)
    is_similar_list = len(similar_list) != 0
    print(similar_list)
    context = {
        "listing": listing,
        ...
        "is_similar_list":is_similar_list
        # "comments": comments
    }
    return render(request, "auctions/listing_page.html", context)

# def recommend(request, listing_id):
#     listing = Listing.objects.get(pk=listing_id)
#     similar_list = Listing.objects.filter(category=listing.category).exclude(pk=listing_id)[:3]  
#     return render(request, "auctions/listing_page.html", {"listing":listing, "similar_list":similar_list})
...
#         return redirect('login')
#     else:
#         df = pd.Dataframe(list(Listing.objects.all().values()))
#         nu = df.user_id.unique().shape[0]
#         curernt_user_id = request.user.id
